...
export class AgentRepo {
  ...
  uniqueName(base: string): string {
    const raw = base.trim() || 'Nouvel agent';
    let name = raw;
    let i = 2;
  ...
  create(input: CreateAgentInput): Agent {
    const now = nowIso();
    const id = uuid();
    const name = this.uniqueName(input.name?.trim() || 'Nouvel agent');
    this.db
      .prepare(
  ...
  delete(id: string): boolean {
    const run = this.db.transaction(() => {
      this.db.prepare(`DELETE FROM agent_inbox WHERE from_agent_id=? OR to_agent_id=?`).run(id, id);
      this.db.prepare(`DELETE FROM channel_members WHERE agent_id=?`).run(id);
      this.db.prepare(`UPDATE channel_messages SET from_agent_id=NULL WHERE from_agent_id=?`).run(id);
      this.db.prepare(`DELETE FROM tasks WHERE agent_id=?`).run(id);
      this.db.prepare(`DELETE FROM approvals WHERE agent_id=?`).run(id);
      this.db.prepare(`DELETE FROM shares WHERE agent_id=?`).run(id);
      this.db.prepare(`UPDATE uploads SET agent_id=NULL WHERE agent_id=?`).run(id);
      ...
      for (const row of lonely) {
        this.db.prepare(`DELETE FROM channel_messages WHERE channel_id=?`).run(row.id);
        this.db.prepare(`DELETE FROM channel_members WHERE channel_id=?`).run(row.id);
        this.db.prepare(`DELETE FROM channels WHERE id=?`).run(row.id);
      }
