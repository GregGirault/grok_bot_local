...
export const OFFICIAL_AVATAR_SHAPES: AvatarShape[] = [
  'circle',
  'blob',
  'rounded',
  'pill',
...
export const OFFICIAL_AVATAR_COLORS = [
  '#F4EFE6',
  '#7A4A28',
  '#FF6A00',
  '#F5C400',
...
export function snapAvatarColor(color: string | undefined | null): string {
  if (!color) return '#FF6A00';
  const raw = color.trim();
  const exact = OFFICIAL_AVATAR_COLORS.find((c) => c.toLowerCase() === raw.toLowerCase());
  ...
  if (!rgb) return '#FF6A00';
  let best = '#FF6A00';
  let bestDistance = Infinity;
  for (const candidate of OFFICIAL_AVATAR_COLORS) {
    const c = hexRgb(candidate)!;
